...
4. **Prediction scan — mandatory.** Re-read the session for every checkable judgment made: a recommendation, a forecast, a call reality will later grade. Each becomes one line with a `verify_by` date (record shape: conventions). **CoachApp session: append to `coachapp/docs/predictions.jsonl`** (moved from the Vault 2026-09-17 — CoachApp's `docs/decisions.md` has the why; `guardrails.mjs` RULE 6 in that repo gates commits on it, and it lands in that repo's own commit, not this ritual's Vault commit at step 6). **Every other project:** `Vault/memory/predictions.jsonl`, as before. This is the learning loop's intake — skipping it is the one lapse the OS cannot afford. "No predictions this session" is a legitimate finding; not looking is not.
