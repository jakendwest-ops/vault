...
_Last updated: 2026-07-10 (session 23)_
...
## 🐛 Session backlog — 2026-07-10 (session 23 — finishing the Workouts-polish build, deep test investigation)
...
|---|---|---|---|---|
| 1 | **BUG — `client_programs` has no client-read RLS SELECT policy** | **🔴 Critical, needs Jake** | **Found, not fixed — needs a Supabase SQL change only Jake can apply** | Confirmed directly: a genuine (non-solo) client account reads back **zero** rows from `client_programs`, even completely unfiltered, while the exact same account correctly reads `workout_logs`/`weight_logs`. Both `app-dashboard.js` and `app-workouts.js` query `client_programs` directly client-side with no other access path — so any real client with an assigned program currently sees "no program assigned" on their Dashboard and Workouts page. Invisible until now because solo accounts share the coach's own `auth.uid()` and never hit this RLS check. Fix: `CREATE POLICY "Clients can view their own program assignments" ON client_programs FOR SELECT USING (client_id IN (SELECT id FROM clients WHERE user_id = auth.uid()));` — run in the Supabase SQL editor. One new Playwright test (`client-workout.spec.js`, hero card DOM test) is `test.fixme()`'d pending this fix, with the logic-level coverage (`_buildWorkoutsHero` unit tests) still passing independently. |
...
| 5 | Runner should round %1RM-calculated target weights down to the nearest 2.5kg | 🟡 Medium | Not built — new feature request | Affects wherever %1RM → kg target is computed/displayed (the strength-table target bar, `_buildTargetCols`/`_renderTargetBarHtml`). Needs scoping: round every displayed target, or just the pre-filled input default (letting the user override with an exact value)? |
| 6 | Plate calculator | 🟢 Low | Already tracked — see Runner features table below | No new documentation needed. |
...
| 3 | "Delete week" button on a phase | Medium | **Ready to build — scoped 2026-07-08** | **Decided:** renumber weeks after the deleted one down by 1 (matches "Duplicate week"'s existing expectation of contiguous slots). No per-week delete exists today — only tail-truncate on regenerate/duration-shrink (`_cleanupPhaseWeeksBeyond`, app-programs.js:1097-1113) — same button lives on the week header, same 4-step cleanup pattern (master `program_phase_workouts` + templates, client-propagated copies + their clones) filtered by `week_number = N` instead of `> maxWeek`. Also decrements `program_phases.duration_weeks`. |
