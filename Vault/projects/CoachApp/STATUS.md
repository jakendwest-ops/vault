# CoachApp — STATUS
_Last updated: 2026-07-03 (session 12)_

---

## Live state

**App version:** app-core/dashboard/clients v=1 · app-programs v=8 · app-calendar-goals v=2 · app-workouts v=9 · app-runner v=9 · app-progress v=3
...
**Last push:** ec30ebf — trivial empty verification commit confirming coachapp pushes still work after the PAT security cleanup (leaked `ghp_` token deleted, embedded token stripped from the remote URL, git now authenticates via the `gh` CLI keyring `gho_` token — see CRITICAL.md 2026-07-03 timeline); full pre-push hook incl. 27/27 Playwright green; no app code. (Previous: 609d5ee — removed dead graphify build-tooling: PreToolUse hooks, `graphify-out/`, obsolete `CLAUDE.md`; no app code, no module-version change; 27/27 Playwright green.) (Previous: 66bf1fd — `deleteProgram()` now blocks (toast, nothing touched) if any clients are assigned to the program; otherwise cleans up the program's own `workout_templates` before deleting (previously left behind as orphaned debris — DB cascade handles phases/phase-workout-slots/template-exercises automatically, confirmed via live FK-cascade research in Supabase before building). Verified live: block path tested directly against Jake's real "Test 1" program (2 clients) — confirmed no data touched; cleanup path tested against a disposable fixture program+phase+template created and torn down in-session. Playwright 56/56. (Previous: dee1479 — fixed two pre-existing scoping bugs found in routine code review: `showLogSessionModal`'s template dropdown had zero `coach_id` scoping, `deleteProgram()` deleted by id with no ownership check. 1914e7b — session 11's runner set-accuracy build pushed after a fresh 3-agent review this session (all clean), one dead-code nit removed.)
...
- **`deleteProgram()` safety + cleanup** — blocks deletion with a toast (no confirm dialog, nothing touched) if any clients are assigned; otherwise deletes the program's own `workout_templates` before removing it, so deleting a program no longer leaves orphaned template debris behind. 2026-07-03
...
| Action | Priority |
|---|---|
| **5 `clients` queries in `app-clients.js` filter by `id` only, no `coach_id`** — openClient (:214), renderClientOverview (:311), saveUpdateEmail (:401), showEditClientModal (:411), saveEditClient (:473). Surfaced by the 2026-07-03 session-start code review. RLS almost certainly blocks cross-tenant access so this is defense-in-depth, not a live leak; add explicit `coach_id` scoping like `deleteTemplate`/`deleteProgram` do. Pre-existing (untouched since the module split) — exactly why ~12 diff-only reviews never looked at it; this is the rationale for the new weekly full-file review pass. | Medium |
...
| **Watch the next few pushes to confirm the GitHub Pages fix holds** — switched Pages deploy source from legacy branch-deploy to Actions-only (`build_type: workflow`) 2026-07-03 after two separate deploy workflows both failed on the same push and generated duplicate failure emails; verified the live site was unaffected immediately after the switch, but durability across future pushes (i.e. the native workflow genuinely never fires again) is only provable by watching it happen. | Low |
