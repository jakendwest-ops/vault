...
**App version:** app-core/dashboard/clients v=1 · app-programs v=6 · app-calendar-goals v=2 · app-workouts v=8 · app-runner v=7 · app-progress v=3
...
### Cache busting
Each of the 8 module files has its own independent `?v=N`. Any commit that changes a module file must bump that file's own version in index.html — bump only the files that changed, not all 8. Current: core/dashboard/clients v=1 · programs v=6 · calendar-goals v=2 · workouts v=8 · runner v=7 · progress v=3.
...
| **`deleteProgram()` doesn't clean up cloned workout_templates — NOW CONFIRMED LIVE (2026-07-03).** Deleting a program cascades away phases/program_phase_workouts but orphans any templates cloned into it (periodization-generated or otherwise). The `— W2/W3/W4` orphan clones we deleted from Jake's account this session were exactly this bug's debris — no longer theoretical. Fix before building more programs: before deleting a program, collect template_ids via program_phase_workouts and delete those (and their workout_template_exercises children) too. Bumped High. | **High** |
| **Trace + clean the ~49 in-use duplicate base templates** — after the 2026-07-03 orphan sweep, `Lower A` (13), `Lower B`/`Upper A`/`Upper B` (12 each) copies remain because each is wired into a `program_phase_workouts` row — almost certainly leftover *test programs* each cloning its own template. The picker still shows them duplicated. Safe cleanup = identify those programs, confirm they're test debris, delete the programs (which will re-orphan the templates once deleteProgram is fixed), then re-run the orphan sweep. Not started — Jake deferred at session end. | Medium |
