...
**App version (last pushed/deployed):** app-core v=1 · app-dashboard v=2 · app-clients v=2 · app-programs v=9 · app-calendar-goals v=2 · app-workouts v=11 · app-runner v=10 · app-progress v=3
**⚠️ Uncommitted local changes (session 17, NOT yet pushed):** app-dashboard v2→v3, app-workouts v11→v12, app-runner v10→v13, app-progress v3→v4 (8 files, +234/-27 lines). Multi-agent review has NOT been run yet — do that before any push. See LOG for full contents.
...
- **Dashboard "Current program" header** — client + solo dashboards now show a small header (program name + "View program" button) above the "Up next" hero card, when a program is assigned. app-dashboard v3, 2026-07-05, **NOT YET PUSHED**.
...
- **Runner fixes** — rest time entered on Swap/Add exercise now actually overwrites the original (was silently falling back to 90s); delete-set button has deliberate spacing from the complete-set tick; mobile RPE/RIR label no longer repeats between header and field; "Save workout" no longer shows a false-positive "Save failed" toast when a non-blocking client-lookup hiccups (found in 3 places: `saveRunnerSession`/`saveWorkoutSession`/`showLogSessionModal`); a real stuck-Save-button bug fixed alongside it. app-runner v10→v13, 2026-07-05, **NOT YET PUSHED**.
...
| Action | Priority |
|---|---|
| **⚠️ Nothing from this session is pushed yet** — 8 files changed (app-dashboard, app-workouts, app-runner, app-progress + tests), sitting uncommitted. Needs multi-agent review before pushing — not run yet this session. | **High** |
| **UNVERIFIED — 3 live checks Jake was mid-way through when the session ended:** (1) Weight goals feature — does saving Starting/Goal weight actually reshape the Body Weight chart's Y-axis; (2) Dashboard "Current program" header — does it show correctly with a working "View program" button; (3) %1RM table routing fix — does Trap Bar Jump now show the strength table (matching Back Squat) instead of the old wizard. All three are Playwright-tested but not yet confirmed by Jake against his own real account/data. | **High** |
