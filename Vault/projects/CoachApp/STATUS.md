# CoachApp — STATUS
_Last updated: 2026-07-03 (session 10)_

---

## Live state

**App version:** app-core/dashboard/clients v=1 · app-programs v=6 · app-calendar-goals v=2 · app-workouts v=9 · app-runner v=8 · app-progress v=3
**Hosting:** GitHub Pages — https://jakendwest-ops.github.io/coachapp
**CSS version:** v=3 (main.css)
**Last push:** 5438aac — Runner + programs fixes from Jake's live test run (13 items). Runner v6→v7: target info restored to the strength table, rest timer moved inline as a 0:00 countdown, voice-cue fix + female voice, visible tick button, mid-workout swap/add exercise (session-only). Programs v5→v6: create-new-workout auto-assigns to its day slot, Edit button on the session-detail drawer, phase card shows the periodization range. Workouts v7→v8: tempo field 4-char cap. Verified: Playwright 51 passed (pre-push hook full suite green) + 3-agent review (security/scoping, solo-mode, duplicates/render-safety — all clean or pre-existing) + 2 new runner smoke tests + live-serving v7 confirmed via curl. NOTE: the "Check & Deploy" CI job showed a red ✗ (transient GitHub Pages deploy race — a second native pages-build-deployment workflow deployed the same commit successfully; site is live). (Previous: 6e6402a — Hevy-style strength table v1.)
**⚠ UNCOMMITTED LOCAL WORK (session 11, 2026-07-03):** `index.html`, `js/app-runner.js` (v7→v8), `js/app-workouts.js` (v8→v9), `tests/runner.spec.js` — runner set-accuracy build + swap/add modal unification (see LOG). Playwright 56/56, but **no multi-agent code review has run and Jake has not said push** — do not assume this is live. Run the review before any push.
...
- **`deleteProgram()` orphan-cleanup fix — scoped, not built.** Re-confirmed via full FK enumeration (`workout_template_exercises.template_id`, `program_phase_workouts.template_id`, `client_program_workouts.workout_template_id`) that `deleteProgram()` only deletes the `programs` row — never its cloned master/periodization-week templates, which is why the assign-picker keeps accumulating debris. Proposed: clean up owned templates on delete; block deletion with a clear message if clients are currently assigned instead of silently orphaning data or failing on a raw FK error. Jake's calls so far: skip building a one-click replace/update-assignment flow for now (just prevent silent double-assign); when calendar integration is built, it should write real per-session `calendar_events` rows, not a lighter marker.
...
### Cache busting
Each of the 8 module files has its own independent `?v=N`. Any commit that changes a module file must bump that file's own version in index.html — bump only the files that changed, not all 8. Current (local, uncommitted — see banner at top): core/dashboard/clients v=1 · programs v=6 · calendar-goals v=2 · workouts v=9 · runner v=8 · progress v=3.
...
| **`deleteProgram()` doesn't clean up cloned workout_templates — CONFIRMED LIVE, root cause re-verified 2026-07-03 (session 11) via full FK enumeration.** Every reference to `workout_templates` traced (`workout_template_exercises.template_id`, `program_phase_workouts.template_id`, `client_program_workouts.workout_template_id`); `deleteProgram()` (app-programs.js:755) only deletes the `programs` row. Fix proposed, not built: clean up owned master + periodization-clone templates on delete; block deletion (clear message) if clients are currently assigned. Awaiting Jake's go-ahead to build. | **High** |
| **Orphaned/duplicate workout_templates keep growing** — same class of debris as the 65 already cleaned 2026-07-02, now recurring with new names (AAA, Full Body A, Low Intensity - Rowerg, Lower A). A read-only diagnostic SQL was handed to Jake this session (per-template reference-count breakdown, scoped to his coach_id) — **awaiting him to run it and paste results back** before any DELETE is drafted. Cleaning without fixing `deleteProgram()` first (row above) will just refill. | Medium |
...
| **Uncommitted runner build needs code review + push decision** — session 11's set-accuracy build + modal unification (app-runner v8, app-workouts v9) is Playwright-verified (56/56) and live-preview verified, but no multi-agent code review has run and nothing has been pushed. Run the review before push. | **High** |
