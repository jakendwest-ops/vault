...
**Additional signals — 2026-07-23 session (exercise-builder overhaul, then day-row prescriptions)**

- **Opens with a UX critique framed as a question, not a bug report.** *"Do you think the apps UI is a bit inconsistent? I do like the exercise builder that we have, however..."* — he leads with a genuine question, praises the thing he's about to criticise, then lands the real concern (copycat risk). Treat the question as sincere and answer it directly with evidence; the "however" is where the actual ask lives. (Casual/AI register.)
- **Names a competitor-similarity worry as a business risk, unprompted, for the second time.** After Hevy (2026-07-13) and SetGraph (07-19), now Heavyset: *"I dont want to get pulled up on being a copycat."* This is a standing concern, not a one-off — expect it on any UI that resembles a known fitness app, and address it in the build rather than waiting to be asked.
- **Bundles several concrete asks into one message and expects all of them done.** One message carried: watts, metres-not-km, optional pace, drop the redundant pace/km line, and "jump height and jump distance are not fleshed out at all". Consistent with the long-banked "both"/"all" = execute everything.
- **Reframes a problem when the proposed fix targets the wrong layer.** Asked where duplicate workouts came from, he answered the *history* question but then corrected the framing: *"it is very likely that a user will have more than 1 of the same workout in a program (especially if they are duplicating weeks) so it gets messy and confusing when they are trying to select the correct workout."* That converted a data-cleanup task into a UI-disambiguation requirement. **When he adds "however"/"it is very likely that…", he is redirecting the problem statement, not adding detail — re-scope before building.**
- **Explicitly asks for a scoping session rather than a fix, when he judges something needs design.** *"The library page is messy and not clear to the user or good UX. this may need a scoping session on its own."* He self-triages build-now vs scope-first; honour the distinction and do not build the scoped item.
- **Sequences work explicitly at a decision point:** *"review and push what is done first, then put yourself into plan mode so we can scope and plan the new issues i have raised to you."* Same release-boundary management as the earlier "commit them but do not push" and "push first then continue".
- **No draft→ship diff this session** (code + planning only; no external-facing prose drafted in his voice).

**Additional signals — 2026-08-09 session (CoachApp: cardio/interval merge, in-app solo-account invites)**

- **Rejects a working solution once he sees its real operational cost, and explicitly authorizes more build effort to remove it.** A local script (functionally correct, one-time key setup) was rejected: *"this is too convoluted and wont be able to be done at scale for me. there must be a simpler way... even if we have to build the solution."* Extends the already-banked "thats way too many steps, please make it more simple" (2026-07-07, same domain — scripts he has to run himself) with a new explicit clause: he'll trade MORE work on Vision's side for LESS operational complexity on his own, stated up front rather than needing to be offered. When a first pass technically works but requires him to operate infrastructure (keys, terminals, manual steps) for something he'll repeat, don't wait for pushback — propose the in-app/automated version first.
- **States a technical gap factually, then tests his own understanding with a direct question.** *"i dont know how to open a terminal in the coach app folder. Is my friend not able to crete an account on the front end?"* — no embarrassment framing, just the fact, immediately followed by checking his own mental model of the system (same shape as the already-banked "opens a bug report as a question against his own mental model," 2026-07-11 — now confirmed in an infra/deployment context, not just product architecture).
- **"your description and steps leave a lot of gap"** — reinforces the already-banked standing rule (2026-06-23/2026-07-03) that manual steps handed to him must be concrete and complete, not summarized; he will call out under-specification bluntly rather than muddle through.
